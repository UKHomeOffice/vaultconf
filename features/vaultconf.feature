Feature: vaultconf can add users with policies to a vault server
  ...
  Scenario: vaultconf can add policies to vault
    ...
    When I do "vaultconf -c test/resources/policies -u user -p password -a http://localhost:8200"
...
#    When I successfully run "vaultconf users /tmp/users.yaml -u user -p password --server http://localhost:8200"
